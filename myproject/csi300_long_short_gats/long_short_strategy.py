...
class TopkDropLongShortStrategy(BaseSignalStrategy):
    ...
    def generate_trade_decision(self, execute_result=None):
        ...
        if len(buy_list) > 0:
            ...
            # 按信号强度分配资金（使用总资产而非可用现金）
            total_score = sum(signal_scores.values())
            if total_score > 0:
                # 每只股票的资金 = 总资产 * 该股票信号强度 / 总信号强度
                stock_values = {
                    code: total_value * self.risk_degree * risk_degree_multiplier * signal_scores[code] / total_score
                    for code in buy_list
                }
            else:
                # 如果总信号强度为0，等权分配
                value_per_stock = total_value * self.risk_degree * risk_degree_multiplier / len(buy_list)
